...
public class RoomCreateHandler extends GuardedMatrixHandler {
    ...
    protected Optional<Response> onRequest(HttpExchange rq, String body, String roomAlias, int pathLength, String[] queryArgs) throws Throwable {
        if (Namespaces.notFoundInAnyNamespace(as.registration.namespaces().aliases(), roomAlias)) return Optional.of(new Response(404, "json", "{\"errcode\":\"M_NOT_FOUND\",\"error\":\"Room did not match this appservice's „aliases:” namespace - assuming it's certainly not gonna be found.\"}"));
